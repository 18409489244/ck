...
class Searcher {
    ...
    [getStartEndPtr](idx, fd, ioStatus) {
        if (this._vectorIndex) {
            ...
            const buf = await
            this[getBuffer](256 + idx, 8, fd, ioStatus)
    ...
    [getBuffer](offset, length, fd, ioStatus) {
        if (this._buffer) {
    ...
    search(ip) {
        ...
        if (!this._buffer) {
            fd = await
            this[openFilePromise](this._dbFile)
        }
        ...
        const {sPtr, ePtr} = await
        this[getStartEndPtr](idx, fd, ioStatus)
        let l = 0
        ...
        while (l <= h) {
            ...
            const buff = await
            this[getBuffer](p, SegmentIndexSize, fd, ioStatus)
            ...
            if (ipInt < sip) {
                ...
                if (ipInt > eip) {
                    ...
                    const data = await
                    this[getBuffer](dataPtr, dataLen, fd, ioStatus)
